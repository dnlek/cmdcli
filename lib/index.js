...
function isRequired(id, cfg) {
  return cfg.required || !id.some(function (item) {
    return item.indexOf('-') === 0;
  });
}

function mapArgparse(id, cfg) {
  var isArgRequired = isRequired(id, cfg);
    ...
    if (params) {
      ...
      try {
        for (var _iterator2 = params[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          var p = (0, _utils.arrayify)(param);
          var cfg = _typeof(p[p.length - 1]) === 'object' ? p.pop() : {};
          var id = Array.isArray(p[0]) ? p[0] : p;
